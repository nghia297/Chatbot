...
with open('./database.json','r', encoding='utf-8') as file:
    db = json.load(file)
with open('./test_intents.json','r', encoding='utf-8') as file:
    intents = json.load(file)
with open('./state_vector.json','r', encoding='utf-8') as file:
    states = json.load(file)
    
# --------------Chuẩn bị data train bag_word & bag_word_state--------------
list_key_db = list(db.keys())    #hiện tại có (học phí, giới thiệu chung, điểm chuẩn, tổ hợp xét tuyển)
list_key_intent = []
...
ignore_char = ['?','!','.',',',"ad", "dạ", "ạ", "ơi", "cho", "hỏi", "về", "làm", "thế", "nào", "có",
...
model = load_model('./test4_model_oke.h5')
model_state = load_model('./test4_model_state_oke.h5')
bot_name = 'Duy Tan University'
...
def get_response(chat,slots,count,prev_state):
    ...
    if prob_state > 0.75 and count[0] == 0:
        ...
        slots[1] = state
    if prob_intent > 0.75:
        if tag == "not_understand" and prev_state[0] == slots[1]:
            print()
        elif tag == "not_understand" and slots[0] in list_key_db and slots[1] in state_tags:
            print()
        else:
            slots[0] = tag
    print(slots)

    if tag == "not_understand" and prev_state[0] == slots[1]: #khi chatbot khong hieu cau tra loi cua cau hoi "ban can hoi thong tin nao cua nganh nay"
        for intent in intents["intents"]:
            if intent['tag'] == "not_understand":
                return f"{random.choice(intent['responses'])}"
    ...
    if slots[0] == "not_understand" and slots[1] not in state_tags:
        for intent in intents["intents"]:
            if intent['tag'] == slots[0]:
                return f"{random.choice(intent['responses'])}"

    if slots[0] == "not_understand" and slots[1] in state_tags: #khi chi nhap ten nganh, mac dinh intent: not_understand
        str = f"Bạn cần hỏi thông tin nào của ngành {slots[1]}?"
        slots[0] = ""
        return str

    if slots[0] == "" and slots[1] in state_tags: #thieu intent
        return f"Bạn cần hỏi thông tin nào của ngành {slots[0]}?"

    if slots[0] in list_key_db and slots[1] == "": #thieu state, intent (học phí, giới thiệu chung, điểm chuẩn, tổ hợp xét tuyển)
        return "Bạn cần hỏi thông tin này của ngành nào?"

    if slots[0] not in list_key_db: #intent không phải (học phí, giới thiệu chung, điểm chuẩn, tổ hợp xét tuyển)
        for intent in intents["intents"]:
            if tag == intent['tag']:
                slots[0] = ""
                return f"{random.choice(intent['responses'])}"

    if slots[0] in list_key_db and slots[1] in state_tags: #intent (học phí, giới thiệu chung, điểm chuẩn, tổ hợp xét tuyển), slot 2 la state
        for intent in intents["intents"]:
